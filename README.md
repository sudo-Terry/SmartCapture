...
**단축키를 눌러도 캡처가 저장되지 않습니다.**
화면 기록 권한 문제입니다. 단축키 충돌은 아닙니다. 이 앱은 `⌃⌥⌘`, 기본 스크린샷은 `⌘⇧`를
사용합니다. 권한을 허용한 뒤 앱을 다시 실행하세요. ad-hoc 서명 특성상 앱을 다시 빌드하면
권한이 초기화될 수 있으며, 자체 서명 인증서로 서명하면 유지됩니다
(`SIGN_IDENTITY="이름" ./build_app.sh`).
